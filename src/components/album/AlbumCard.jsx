...
export default function AlbumCard({ album, userAlbum, variant, onMarkListened, onRevertPending, onDelete, isProcessing }) {
  ...
  return (
    <div className="bg-gray-900 rounded-xl overflow-hidden flex sm:flex-col">
      {/* Portada */}
      <div className="w-24 h-24 sm:w-full sm:aspect-square flex-shrink-0">
        {album.cover_url ? (
          <img
            src={album.cover_url}
            alt={album.name}
      ...
      <div className="flex-1 p-3 sm:p-4 flex flex-col justify-between min-w-0">
        ...
        <div className="flex items-center gap-2 mt-3 flex-wrap">
          ...
          >
            {variant === 'listened' ? 'Escuchar de nuevo' : 'Escuchar en Spotify'}
          </a>
          ...
          <button
            ...
            disabled={isProcessing}
            className="text-xs text-gray-500 hover:text-red-400 transition-colors ml-auto disabled:opacity-50 disabled:cursor-not-allowed"
            aria-label="Eliminar"
          >
            ✕
          </button>
